...
def pad_feature_first(batch: List[Union[torch.Tensor, List[torch.Tensor]]], padding_type='post') -> List[torch.Tensor]:
    """
...
class ZeroPadding:
    def __init__(
        ...
        :param feature_first: Whether the batch is in feature-first format.
    ...
    def pad(self, batch: List[Union[List[Union[torch.Tensor, np.ndarray]], Union[torch.Tensor, np.ndarray]]]) -> Union[
        torch.Tensor, List]:
        """
        Pad a batch of sequences to the same length with the specified padding value,
        handling both the original and new input formats based on self.feature_first.
        Can optionally return original lengths and labels.

        :param batch: A list of sequences or a nested list of sequences for each feature across samples.
        ...
        if self.feature_first:
            if self.fixed_length:
                raise NotImplementedError("Fixed length padding is not supported for feature-first format.")
            # Handling input format (batch_size, num_features, length, feature_dim)
            transposed_batch = list(zip(*batch))  # Transpose to (num_features, batch_size, length, feature_dim)
            # Print length in 4 steps
            # print("Lengths: ", len(transposed_batch), len(transposed_batch[0]), len(transposed_batch[0][0]), len(transposed_batch[0][0][0]))

            if self.return_len:
                lengths = [len(e) for e in transposed_batch[0]]

            padded_features = []
            for feature in transposed_batch:
                tensors = [torch.from_numpy(e) if isinstance(e, np.ndarray) else e for e in feature]
                padded_feature = pad_sequence(tensors, batch_first=True, padding_value=self.padding_value)
                padded_features.append(padded_feature)

            batch = padded_features  # (num_features, batch_size, max_sequence_length, feature_dim)
