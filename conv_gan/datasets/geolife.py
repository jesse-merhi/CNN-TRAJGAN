...
import config
from . import DictProperty, TrajectoryDataset, DatasetModes, SPATIAL_COLUMNS
from . import preprocessing as pp
from ..utils.data import normalize_points
from ..utils.helpers import find_bbox, get_ref_point, get_scaling_factor
...
NUM_PROCESSED_TRAJS = None  # 15991  # See geoLife.ipynb
SAMPLING_RATE = 5  # [s]
...
DEFAULT_DATASET_PATH = config.BASE_DIR + "data/geolife/restricted_geolife.csv"
...
class GeoLifeDataset(TrajectoryDataset):
    ...
    def __init__(self, mode: DatasetModes = DatasetModes.PATH, path: str = None,
                 spatial_columns: list = SPATIAL_COLUMNS, latlon_only: bool = True, normalize: bool = True,
                 return_labels: bool = False, sort: bool = False, keep_original: bool = False,
                 as_dataframe: bool = False, reference_point: List[float] or dict = DEFAULT_REF_POINT,
                 scale_factor: List[float] or dict = DEFAULT_SCALE_FACTOR, tids: Optional[List[str]] = None):
        # Validate inputs
        if sort:
            raise NotImplementedError("Sorting is not implemented due to size of GeoLife dataset.")

        # Call superclass / store parameters
        features = ['latlon'] if latlon_only else ['latlon', 'hour','day']
        ...
        if tids is not None:
            self.df = data[data['tid'].isin(tids)]
        else:
            self.df = data
        self.tids = self.df['tid'].unique()
    ...
    def _encode(self, tid: str) -> None:
        ...
        if self.as_dataframe:
            ...
            if self.latlon_only:
                ...
                hour = torch.from_numpy(np.eye(24)[df['hour'].to_numpy()])
                day = torch.from_numpy(np.eye(7)[df['day'].to_numpy()])
