...
import config
from conv_gan.datasets.base_dataset import TrajectoryDataset, DatasetModes, SPATIAL_COLUMNS
from conv_gan.datasets.fs_nyc import FSNYCDataset, PATH_ALL
from conv_gan.datasets.padding import pad_feature_first
from conv_gan.models.trajGAN import TrajGAN
from conv_gan.models.traj_loss import get_trajLoss
from conv_gan.models.utils import l1_regularizer
from conv_gan.utils import logger, helpers
from conv_gan.utils.helpers import count_parameters_torch
...
class LSTM_TrajGAN(TrajGAN):
    def get_noise(self, batch_size: int) -> torch.Tensor:
        return torch.randn(size=(batch_size, self.latent_dim), device=self.device)
...
if __name__ == '__main__':
    ...
    if opt.summary:
        ...
        dataset = FSNYCDataset(mode=DatasetModes.TRAIN, latlon_only=False, normalize=True, return_labels=False,
                               reference_point=ref, scale_factor=sf)
