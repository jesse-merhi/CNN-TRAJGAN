...
import config
from conv_gan.utils import visualise
...
def compute_gradient_penalty(discriminator: nn.Module, real: Tensor, synthetic: Tensor,
                             lengths: list or None = None, lp: bool = False,
                             **kwargs) -> Tensor:
    """
    ...
    :param kwargs: Additional keyword arguments that are passed to the discriminator.
    ...
    if isinstance(real, Tensor) and isinstance(synthetic, Tensor):
        ...
        if real.dim() == 3:
            ...
            alpha = torch.rand(batch_size, 1, 1, 1).to(device=device)
        ...
        for i, feature in enumerate(real):
            ...
            interpolates.append(interpolation.requires_grad_(True).to(device))
    else:
    ...
    with context:
        ...
        if lengths is None:
            d_interpolates = discriminator(interpolates, **kwargs)
        else:
            d_interpolates = discriminator(interpolates, lengths=lengths, **kwargs)
    ...
    grad_outputs = torch.ones_like(d_interpolates, requires_grad=True).to(device=device)
...
def split_data(data: torch.Tensor or list or tuple) -> (torch.Tensor, List[int] or None, list or None):
    ...
    if torch.is_tensor(data):
        ...
        if len(data) == 3 and not (isinstance(data[1], torch.Tensor) and data[1].dim() == 3):
            # The part captures the scenario that with have a list of three features.
...
def visualize_trajectory_samples(gen_samples: torch.Tensor, real_samples: torch.Tensor, epoch: int, batch_i: int,
                                 real_lengths: List[int] = None, gen_lengths: List[int] = None,
                                 tensorboard: bool = False, writer: Optional[SummaryWriter] = None,
                                 display_handle: Optional[DisplayHandle] = None) -> None:
    """
    ...
    :param display_handle: Display handle for updating the plot in a Jupyter notebook. Defaults to None.
...
def determine_device(gpu: int) -> torch.device:
    """
    Determine the device to use for training.

    :param gpu: The GPU to use. If None, use CPU.
    :return: The determined device.
    """
